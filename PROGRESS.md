...
- **任务**：解决 LinX 断线后长期停留在 `peripheralDisconnecting`，以及只按有效血糖判断连接存活的问题。
- **核心交付**：
  1. 蓝牙管理器增加永久关闭能力；60 秒仍未完成握手时先彻底关闭旧管理器，再清除旧蓝牙标识并按已保存序列号创建新管理器继续扫描。
  2. F001、F002、F003 在配对、解密和解析前报告数据包活动；已接受握手后启动 5 分钟检查，任意当前数据包都会重新计时。
  3. 5 分钟没有数据包时主动断开并启动唯一的 60 秒恢复；数据包检查与恢复周期互斥，旧管理器、旧传感器和迟到任务不能改变新连接。
  4. 删除原 15 分钟无有效血糖主动断开逻辑；只有实时血糖更新 `lastReadingDate`，历史、维护、无效和重复数据不改变最后实时血糖时间。
  5. 修复关闭日志死锁、异步启动竞态、重复扫描重置计时、锁内创建蓝牙对象、队列身份地址复用和两项异步测试偶发失败。
- **验证结果**：`MicroTechCGMTests` 全量 246 项独立通过，并连续三轮共 738 项通过、0 失败；队列身份和真实蓝牙重复激活测试各重复 100 次，共 200 项通过；`MicroTechCGMManagerTests` 连续五轮共 905 项通过；`LoopWorkspace` 无签名 Simulator 完整构建在最终代码上再次通过。首次完整构建被全局失效代理 `127.0.0.1:1081` 阻断，当前命令临时改用 `socks5h://127.0.0.1:1080` 后通过，未修改全局配置。最终规格审查已批准。
- **关键发现**：5 分钟 `monitoring` 与 60 秒 `timing`、`shuttingDown` 必须互斥；握手需要在同一次状态变更中取消恢复并建立数据包检查，避免瞬时并发窗口。
- **决策结论**：自动化实现和构建已完成；真实 LinX 正常数据持续 5 分钟不误重建，以及真实 `peripheralDisconnecting` 卡死后的 60 秒重建仍待真机复验。本次未安装真机包、未上传 TestFlight。
- **commit hash**：`f385558` 至 `0830319`；验证文档提交 `2bb4b2a`、`0058bb0`。
- **push 状态**：已合并并推送到 `origin/main`；本条状态随最终提交同步。

### 2026-08-03 045 - 编写 LinX 数据包静默重连实施计划

- **任务**：把已确认的 LinX 5 分钟数据包静默检查和 60 秒蓝牙重建整理为可直接执行的实施计划。
- **核心交付**：
  1. 新增 `docs/superpowers/plans/2026-08-03-linx-packet-silence-reconnect.md`，分为蓝牙管理器关闭与 60 秒恢复、原始通知与 5 分钟检查、完整回归与交付三个部分。
  2. 明确旧蓝牙管理器永久关闭、manager generation、关闭完成后的原子激活、删除和模式切换竞态、旧 manager/sensor 回调隔离及无状态锁交叉等待要求。
  3. 明确 F001、F002、F003 在解密和解析前上报活动；只有实时血糖更新 `lastReadingDate`，有效历史数据仍可输出但不能覆盖最后实时血糖时间。
  4. 每项代码改动均要求先运行整组新增失败测试，再实现并运行同组通过测试；最终执行 LinX 全量测试、完整 workspace 构建、独立审查和推送前真机安全检查。
- **验证结果**：实施前 `MicroTechCGMTests` 基线 193 项通过、0 失败；完整 `LoopWorkspace` 基线被仓库已忽略的本地 `LibreTransmitter/LibreTransmitter/NotificationHelperOverride.swift` 缺失阻断，计划已加入临时生成默认文件、确认不被 Git 跟踪并在构建后删除的步骤；计划经过三轮独立审查后通过；`git diff --check` 通过。
- **决策结论**：5 分钟检查使用独立数据包时间；60 秒超时先永久关闭旧 manager，再创建带新代次的 manager；生产代码实现和真机验证待执行。
- **commit hash**：`6909c9e`。
- **push 状态**：已推送到 `origin/main`。

### 2026-08-03 044 - 补充 LinX 5 分钟数据包静默检查设计

- **任务**：把 LinX 连接后的无通信检查从 15 分钟无血糖改为 5 分钟无任何数据包。
- **核心交付**：
  1. 连接完成后开始 5 分钟检查，当前连接收到 F001、F002 或 F003 数据包时重新计时。
  2. 数据包无需成功解密、解析或生成有效血糖；维护包、历史包和无效包都能证明连接仍有通信。
  3. 数据包活动时间与 `lastReadingDate` 分开，避免改变用户看到的最后血糖时间。
  4. 旧连接数据包、删除 CGM、正常断开和广播兼容模式不会刷新或触发检查。
- **验证结果**：设计已由用户确认；独立审查发现的首次计时起点、原始通知顺序和并发断开边界已补齐，复审通过；`git diff --check` 通过；代码、单元测试和真机验证待完成。
- **决策结论**：采用独立的最后数据包时间和可失效检查标识；连续 5 分钟没有任何数据包时主动断开并进入既定重连流程。
- **commit hash**：`5f17434`、`8a60fff`。
- **push 状态**：已推送到 `origin/main`。
